...
func TestCreateAccount(t *testing.T) {
	...
	err := json.NewEncoder(&goodBody).Encode(createAccountRequest{
		Owner:    account.Owner,
		Currency: account.Currency,
	})
	require.NoError(t, err)

	var goodBody2 bytes.Buffer
	err = json.NewEncoder(&goodBody2).Encode(createAccountRequest{
		Owner:    account.Owner,
		Currency: account.Currency,
	})
	require.NoError(t, err)
	...
	err = json.NewEncoder(&badBody).Encode(createAccountRequest{
		Owner:    "111",
		Currency: "RUB",
	})
	require.NoError(t, err)
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			request, err := http.NewRequest(http.MethodPost, url, tc.reqBody)
			require.NoError(t, err)
